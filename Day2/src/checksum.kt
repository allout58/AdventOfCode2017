...
//            "9 4 7 3\n" +
//            "3 8 6 5"
    val input = "1236\t741\t557\t1029\t144\t101\t1968\t2159\t1399\t80\t1139\t1167\t1695\t82\t90\t2236\n" +
    ...
    val sheet = input.split("\n").map { it.split(Regex("\\s")).map { it.toInt() } }
//    Part 1
//    println(sheet.map { (it.max() ?: 0) - (it.min() ?: 0) }.reduce(Int::plus))
    println(sheet.map {
        it.distinct().mapNotNull { v -> mapDivisor(v, it) }.first()
    }.reduce(Int::plus))
}

fun mapDivisor(value: Int, row: List<Int>): Int? {
    val div = row.find { value != it && value % it == 0 }
    if (div != null) {
        return value / div
    }
    return null
